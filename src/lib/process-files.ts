...
/**
 * Process Word file — just read the ArrayBuffer.
 * Actual conversion is done server-side (Gotenberg/LibreOffice)
 * during the merge step. Page count is updated after conversion.
 */
export async function processWordFile(fileData: ProcessedFile): Promise<void> {
  ...
  fileData.pages = -1; // Will be determined during merge
}
